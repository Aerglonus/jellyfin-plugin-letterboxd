        ...
        foreach (var user in e.Users)
        {
            ...
            try
            {
                ...
                if (diary.LastDate != null && diary.LastDate.Value.Date == viewingDate)
                {
                    ...
                    await SyncHelpers.SyncFavoritesAsync(client, account, film.FilmId, ud?.IsFavorite ?? false, e.Item.Name, _logger).ConfigureAwait(false);
                ...
                await SyncHelpers.SyncFavoritesAsync(client, account, film.FilmId, userData?.IsFavorite ?? false, e.Item.Name, _logger).ConfigureAwait(false);
