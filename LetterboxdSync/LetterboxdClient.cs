...
public class LetterboxdClient : IDisposable
{
    ...
    public Task<List<int>> GetWatchlistTmdbIdsAsync(string username)
    {
        ...
        return GetTmdbIdsFromPagedApiAsync(
            $"api/v0/member/{_memberId}/watchlist?sort=Added&perPage=100",
            item => item.TryGetProperty("links", out var links) ? links : null
        );
    }
    ...
    public Task<List<int>> GetDiaryTmdbIdsAsync(string username)
    {
        ...
        return GetTmdbIdsFromPagedApiAsync(
            $"api/v0/log-entries?perPage=100&member={_memberId}&memberRelationship=Owner&where=HasDiaryDate",
            item => item.TryGetProperty("film", out var filmEl) && filmEl.TryGetProperty("links", out var links) ? links : null
        );
    }

    private async Task<List<int>> GetTmdbIdsFromPagedApiAsync(string urlTemplate, Func<JsonElement, JsonElement?> getLinksElement)
    {
        ...
        do
        {
            ...
            if (doc.RootElement.TryGetProperty("items", out var items))
            {
                ...
                {
                    var linksEl = getLinksElement(item);
                    if (linksEl.HasValue && linksEl.Value.ValueKind == JsonValueKind.Array)
                    {
                        foreach (var link in linksEl.Value.EnumerateArray())
                        {
