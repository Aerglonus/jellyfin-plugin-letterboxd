const PluginId = '00f8df7c-b110-4529-8e8c-7675905cfd0f';
let jellyfinUsers = [];
...
function accountHtml() {
	return (
		'<div class="account-entry verticalSection" style="border: 1px solid #333; padding: 1em; margin-bottom: 1em; border-radius: 4px;">' +
...
function addAccountEntry(account) {
	...
	if (account) {
		...
		entry.querySelector('.syncFavorites').checked =
			account.SyncFavorites === true;
		entry.querySelector('.enableDateFilter').checked =
	...
	entry
		.querySelector('.enableDateFilter')
		.addEventListener('change', function () {
			entry.querySelector('.dateFilterDaysContainer').style.display = this
				.checked
				? ''
				: 'none';
		});
	...
	entry
		.querySelector('.removeAccountBtn')
...
function collectAccounts() {
	...
	entries.forEach(function (entry) {
		accounts.push({
			...
			DateFilterDays:
				parseInt(entry.querySelector('.dateFilterDays').value, 10) || 7,
		});
	});
	return accounts;
}
